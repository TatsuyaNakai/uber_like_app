class Api::V1::LineFoodsController < ApplicationController
  ...
  def create
    if LineFood.active.other_restaurant(@ordered_food.restaurant_id).exist?
      # 注文した商品のお店以外で、activeがtrueなものの配列がある場合は以下を実行する。
      return render json: {
        existing_restaurant: 
          LineFood.other_restaurant(@ordered_food.restaurant.id).first.restaurant.name,
          # existing_restaurantは、注文したお店以外の配列の先頭の名前にする。
        new_restaurant: 
  ...
  def set_line_food(ordered_food)
    if ordered_food.line_food.present?
      ...
      @line_food = ordered_food.build_line_food(
        # 仮注文がなかった場合は、orderd_foodのline_foodを作成する。
        count: params[:count],
